...
const logfile = path.join(__dirname, '../public/boards-splitted/surah-18_part2-splitted-log.json');

// Base string (max allowed length for text_uthmani)
const baseString = "وَمُنذِرِينَ‌ۚ وَيُجَـٰدِلُ ٱلَّذِينَ كَفَرُواْ بِٱلْبَـٰطِلِ لِيُدْحِضُواْ بِهِ ٱلْحَقَّ‌ۖ وَٱتَّخَذُوٓاْ ءَايَـٰتِى وَمَآ أُنذِرُواْ هُزُوًا";
...
✅ Each segment does not exceed ${maxLen} characters.
✅ Strictly return only valid JSON without any additions and/or explanations without \`\`\`json\\n
✅ Strictly return only valid JSON without any explanations.
✅ Return only raw JSON without any markdown formatting or code fences.


Segmentation Rules:
Always prioritize complete sentences.

If a sentence fits within ${maxLen} characters, keep it whole.
...
If the remaining text is ≤${maxLen} characters, return only the final index.
...
`;
  ...
  // Your goal is to split text (first translated, then accordingly the arabic and transcribed text) into multiple segments without exceeding ${maxLen} characters per segment.
  ...
  //     }
    ...
    const destDir = path.dirname(logfile);
    if (!fs.existsSync(destDir)) {
      fs.mkdirSync(destDir, { recursive: true });
    }
    fs.writeFileSync(logfile, JSON.stringify(responseText, null, 2), 'utf8');
...
async function processFile() {
  try {
    ...
    const newVerses = await Promise.all(jsonData.map(async (verse) => {
      const newVerse = {
